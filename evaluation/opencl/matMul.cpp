...
void printMatrix(float* matrix, const char* filename) {
...
void MatrixMulOpenCL(float* M, float* N, float* P, int width) {
  ...
  for(unsigned i = 0; i < runs; ++i) {
    ...
    err = clEnqueueNDRangeKernel(commandQueue, kernel, 2, NULL, globalSize, NULL, 0, NULL, &event);
    checkError(err);
...
int main(void) {
  ...
  printMatrix(P_seq, "opencl_host");
  printMatrix(P_opencl, "opencl_device");
  return 0;
}
