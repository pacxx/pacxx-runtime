...
bool CUPTIProfiler::checkStage(unsigned required, std::string stagename, bool minonly) {
	if (!enabled()) return false;
	if (!minonly && (required < stage)) {
		__error("CUPTIProfiler is already past stage ", required, ", can't run ", stagename);
		return false;
	}
	else if (required > stage) {
		__error("CUPTIProfiler is not yet ready for stage ", required, ", can't run ", stagename);
		return false;
	}
	else return true;
}
...
void CUPTIAPI CUPTIProfiler::bufferCompleted(CUcontext ctx, uint32_t streamId, uint8_t *buffer, size_t size, size_t validSize) {
...
void CUPTIAPI CUPTIProfiler::getMetricValueCallback(void *userdata, CUpti_CallbackDomain domain, CUpti_CallbackId cbid, const CUpti_CallbackData *cbInfo) {
	...
	if (cbInfo->callbackSite == CUPTI_API_ENTER) {
		...
		SEC_CUPTI_CALL(cuptiSetEventCollectionMode(cbInfo->context, CUPTI_EVENT_COLLECTION_MODE_KERNEL));

		for (i = 0; i < metricData->eventGroups->numEventGroups; i++) {
			uint32_t all = 1;
			SEC_CUPTI_CALL(cuptiEventGroupSetAttribute(metricData->eventGroups->eventGroups[i], CUPTI_EVENT_GROUP_ATTR_PROFILE_ALL_DOMAIN_INSTANCES, sizeof(all), &all));
			SEC_CUPTI_CALL(cuptiEventGroupEnable(metricData->eventGroups->eventGroups[i]));
		}
	}
	...
	if (cbInfo->callbackSite == CUPTI_API_EXIT) {
		...
		for (i = 0; i < metricData->eventGroups->numEventGroups; i++) {
			...
			SEC_CUPTI_CALL(cuptiEventGroupGetAttribute(group, CUPTI_EVENT_GROUP_ATTR_EVENT_DOMAIN_ID, &groupDomainSize, &groupDomain));
			SEC_CUPTI_CALL(cuptiDeviceGetEventDomainAttribute(metricData->device, groupDomain, CUPTI_EVENT_DOMAIN_ATTR_TOTAL_INSTANCE_COUNT, &numTotalInstancesSize, &numTotalInstances));
			SEC_CUPTI_CALL(cuptiEventGroupGetAttribute(group, CUPTI_EVENT_GROUP_ATTR_INSTANCE_COUNT, &numInstancesSize, &numInstances));
			SEC_CUPTI_CALL(cuptiEventGroupGetAttribute(group, CUPTI_EVENT_GROUP_ATTR_NUM_EVENTS, &numEventsSize, &numEvents));
			eventIdsSize = numEvents * sizeof(CUpti_EventID);
			eventIds = (CUpti_EventID *)malloc(eventIdsSize);
			SEC_CUPTI_CALL(cuptiEventGroupGetAttribute(group, CUPTI_EVENT_GROUP_ATTR_EVENTS, &eventIdsSize, eventIds));

			valuesSize = sizeof(uint64_t) * numInstances;
			values = (uint64_t *)malloc(valuesSize);

			for (j = 0; j < numEvents; j++) {
				SEC_CUPTI_CALL(cuptiEventGroupReadEvent(group, CUPTI_EVENT_READ_FLAG_NONE, eventIds[j], &valuesSize, values));
				if (metricData->eventIdx >= metricData->numEvents) {
					__error("too many events collected, metric expects only ", (int)metricData->numEvents);
					return;
				}
				...
				// print collected value
				{
					...
					SEC_CUPTI_CALL(cuptiEventGetAttribute(eventIds[j], CUPTI_EVENT_ATTR_NAME, &eventNameSize, eventName));
					eventName[127] = '\0';
					...
					eventlog << ")\n";
					eventlog << "\t" << eventName << "(normalized) (" << (unsigned long long)sum << " * " << numTotalInstances << ") / " << numInstances << " = " << (unsigned long long)normalized;
		...
		for (i = 0; i < metricData->eventGroups->numEventGroups; i++) SEC_CUPTI_CALL(cuptiEventGroupDisable(metricData->eventGroups->eventGroups[i]));
	}
}
...
void CUPTIProfiler::profileSingle(const std::string& metricName) {
	...
	SEC_CUPTI_CALL(cuptiMetricCreateEventGroupSets(static_cast<CUDAKernel*>(_kernel)->getRuntime().getContext(), sizeof(metricId), &metricId, &passData));
	...
	if (metricData.eventIdx != metricData.numEvents) {
		__error("expected ", metricData.numEvents, " metric events, got ", metricData.eventIdx);
	}
	else {
		// use all the collected events to calculate the metric value
		SEC_CUPTI_CALL(cuptiMetricGetValue(_device, metricId, metricData.numEvents * sizeof(CUpti_EventID), metricData.eventIdArray, metricData.numEvents * sizeof(uint64_t), metricData.eventValueArray, kernelDuration, &metricValue));
...
std::string CUPTIProfiler::stringSingleMetric(const std::pair<CUpti_MetricValue, CUpti_MetricValueKind>& entry) {
	...
	switch (entry.second) {
		...
		case CUPTI_METRIC_VALUE_KIND_PERCENT:
			buffer << entry.first.metricValuePercent; buffer << "%";
			break;
		case CUPTI_METRIC_VALUE_KIND_THROUGHPUT:
			buffer << (unsigned long long)entry.first.metricValueThroughput; buffer << "b/s";
			break;
		case CUPTI_METRIC_VALUE_KIND_UTILIZATION_LEVEL:
			buffer << "ul "; buffer << (unsigned int)entry.first.metricValueUtilizationLevel;
			break;
		default:
