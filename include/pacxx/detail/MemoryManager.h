...
public:
  ...
  RawDeviceBuffer &manageVector(const std::vector<T, Allocator> &vec) {
    ...
    if (!buffer) {
      ...
      size_t additonal_bytes = std::max(sizeof(std::vector<T>),
                                        _runtime.getPreferedMemoryAlignment());
      ...
      fake_vector.begin =
          reinterpret_cast<char *>(buffer->get(additonal_bytes));
      fake_vector.end = fake_vector.begin + vector_memsize;

      buffer->uploadAsync(&fake_vector, sizeof(fake_vector),
                          additonal_bytes - 32); // TODO: fix the 32 here
  ...
  RawDeviceBuffer &translateVector(const std::vector<T, Allocator> &vec) {
    ...
    if (!ptr)
      throw common::generic_exception(
          "unmanaged vector instance provided to translateVector");
    return *ptr;
  }
  ...
  void unmanageVector(const std::vector<T, Allocator> &vec) {
    ...
    if (ptr) {
      ...
      __error("unmanaged vector supplied");
  }

  template <typename L> RawDeviceBuffer &getTemporaryLambda(const L &lambda) {
