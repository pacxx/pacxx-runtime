...
namespace pacxx {
namespace cuda {
template <class T> struct CUDAPinnedAllocator {
  ...
  void deallocate(T *ptr, std::size_t n) { SEC_CUDA_CALL(cudaFreeHost(ptr)); }
};
template <class T, class U>
bool operator==(const CUDAPinnedAllocator<T> &,
                const CUDAPinnedAllocator<U> &) {
  return true;
};
...
template <class T> struct CUDAManagedAllocator {
  ...
  template <class U>
  CUDAManagedAllocator(const CUDAManagedAllocator<U> &other) {}

  T *allocate(std::size_t n) {
    T *ptr = nullptr;

    SEC_CUDA_CALL(
        cudaMallocManaged(&ptr, n * sizeof(T) + 256, cudaMemAttachGlobal));
