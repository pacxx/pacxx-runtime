...
namespace pacxx {
    ...
    template<typename T>
    class NativeDeviceBuffer: public DeviceBuffer<T> {
      friend class NativeRuntime;

    private:
      NativeDeviceBuffer(NativeRawDeviceBuffer buffer) : _buffer(std::move(buffer)) {}
    ...
    public:
      ...
      NativeDeviceBuffer(NativeDeviceBuffer&& rhs) { _buffer = std::move(rhs._buffer); }
